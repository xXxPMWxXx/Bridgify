...
export const update = async (req: any, res: Response, next: NextFunction) => {
  try {
    ...
    jwt.verify(token, jwt_secret, async (err: any, decoded: any) => {
      ...
      if (decoded) {
        const { elderlyID, dateTime } = req.body;
        // 4. check if the post is existed
        const post = await RecordModel.findOne({ 'elderlyID': elderlyID, 'dateTime': dateTime });
        if (post == null) {
          return res
            .status(400)
        ...
        try {
          await RecordModel.updateOne({ 'elderlyID': elderlyID, 'dateTime': dateTime }, req.body);
